 ...
 - 2) 커스텀 디코딩
   - +@ optional 대응
     만약 서버에서 price가 Int? 일때 아래와 같이 decodeIfPresent로 해결하면 옵셔널로 값이 들어오더라도 옵셔널 바인딩을 사용하기에 나의 responseModel은 Int로 설정해도 된다.
         ex) mall의 decodeIfPresent와 같이 사용하여 vc에서 할 옵셔널 멘트 대응을 model에서 다룰 수 있음.
 */
struct Product: Decodable {
    ...
    init(from decoder: any Decoder) throws {
        ...
        mall = try container.decodeIfPresent(String.self, forKey: .mall) ?? "쇼핑몰 정보없음"
